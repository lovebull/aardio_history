...
declare namespace aardio {
    ...
    interface External {
        ...
        /** 调用 aardio 函数(即使在aasdl初始化以前仍然可以调用rpc函数) */
        xcall(method: string, ...args: any[]): Promise<any>;
        /** 注册 aardio 可调用的JS函数，如果同一进程内注册多个同名回调，仅回传最后一次回调的返回值，非 aardio 环境自动忽略不执行 */
        ...
        /** 退出进程 */
        quit: () => void;
        /** 拖动标题栏，非 aardio 环境自动忽略不执行 */
        hitCaption: () => Promise<void>;
        /** 点击关闭窗口按钮，非 aardio 环境自动忽略不执行 */
        hitClose: () => Promise<void>;
        /** 点击最小化按钮，非 aardio 环境自动忽略不执行 */
        hitMin: () => Promise<void>;
        /** 点击最大化按钮，非 aardio 环境自动忽略不执行 */
        hitMax: () => Promise<boolean>;
        /** 窗口是否最大化，非 aardio 环境自动忽略不执行 */
