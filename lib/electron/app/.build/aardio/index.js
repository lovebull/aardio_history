...
(function (name, definition) {
  ...
  if (aardio.browser && !startEnviron) {
    ...
    if (window) {
      ...
      rpcAasdl = '{{{$rpcAasdl}}}';//必须使用单引号包含此模板参数
  ...
  if (!aardio.browser) {
    ...
    createWebSocket = () => new WebSocket(startEnviron.rpcMainServerUrl);
    ...
    createWebSocket = () => new WebSocket(wsRpcServerUrl);
  }
  ...
  aardio.open = function(){
    if(aardio.rpcClientId) return;
  ...
  if( rpcAasdl ) aasdlParse(rpcAasdl, aardio);
